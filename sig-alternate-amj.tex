...
We evaluated our channel by comparing it to a normal TLS channel.
\end{abstract}
...
This project aims to solve the main problem originated by having only one cipher suite negotiated between client and server.
When one of the cipher suite's mechanisms becomes insecure, the secure communication channels using that cipher suite may become vulnerable.
Unlike TLS communication channels, superTLS does not rely in only one cipher suite.
Using superTLS, more than one cipher suite is negotiated between client and server and, consequently, more than one encryption mechanism will be used for each purpose -- key exchange, authentication, encryption and MAC.

Although most cipher suites used by TLS 1.2 are regarded as secure, as stated in Section \ref{sec-related-work}, there is not any assurance that a governmental agency or a company with high computational power and financial resources is not able to break one of the encryption mechanisms in the near future.
