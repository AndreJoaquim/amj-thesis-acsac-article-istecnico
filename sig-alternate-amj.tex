...
In order to evaluate SuperTLS, we considered OpenSSL 1.0.2g as the comparison factor, due to the fact that SuperTLS is based on that version of SuperTLS. In the worst case, users should experience a connection $k, k = 2$ times slower than using OpenSSL.
...
\item{The overhead generated by using multiple ciphers and signatures is... <complete this>}

\item{Additional measurements: Measure the throughput of the channel. How many messages are sent in 1 minute in SuperTLS? How many messages are sent in OpenSSL? The throughput should naturally be inferior.}
\item{}
\end{enumerate}
